...
class CCurrency extends ServiceClass {
    ...
    const BASIC_PERMISSION = self::CURRENCIES_READ;
    ...
    static function init($reset) {
		...
		try {
			if (AppDB::query($q)) {
                $cur = new CCurrency("Base Currency","1", (self::FLAG_DEFAULT | self::FLAG_VIRTUAL), "Desc");
                $cur->symbol="$";
                $cur->decimalSymbol=".";
                $cur->groupSymbol=",";
                $cur->trailingZeros="0";
                $cur->visibility="*";
                $cur->description = "This is the default system currency";
                $cur->creator = "System";
                if ($cur->create()) {
                    AppDB::commit();
