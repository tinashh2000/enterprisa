...
use const Tasks\modulePrivilegesBase;

class CTask extends ServiceClass {
    ...
    protected static $forensicPath = "Assistant_Task";


    static $orderByParams = array("name", "description");
    ...
    static function init($reset) {
        ...
        try {
            if (AppDB::query($q)) {
                ...
                return CStatus::pushStatus("Task created successfully");
            }
        } catch (Exception $e) {
        }
        return CStatus::pushError("Task creation failed");
    }
    ...
    function create() {
        ...
        if ($this->exists()) {
            CStatus::jsonError("Task already exists");
        }
        ...
        if ($res = AppDB::query($q)) {
            ...
            self::uploadPicture(true, "pics", $this->id."_pp");
            CForensic::log(self::$forensicPath, "Create Task", $this->id, json_encode($this));
            AppDB::commit();
            return CStatus::jsonSuccess("Task created successfully");
        }
        AppDB::rollback();
        return CStatus::jsonError("Task creation failed");
    }
    ...
    function edit()
    {
        ...
        if ($res = AppDB::query("UPDATE " . self::$defaultTable . "  SET
            ...
            $this->id = AppDB::last_id($res);
            self::uploadPicture(true, "pics", $this->id."_pp");
            CForensic::log(self::$forensicPath, "Edit Task", $this->id, json_encode($this));
            AppDB::commit();
            return CStatus::jsonSuccess("Task updated successfully");
        }
        AppDB::rollback();
        return CStatus::jsonError("Task update failed");
    }
